...
class PVS1CNV:
    ...
    def __init__(self, cnvrecord, consequence, transcript, genome_version):
        ...
        if self.transcript:
            ...
            if self.type in ['DEL', 'del', 'Del']:
                self.strength_raw = self.verify_DEL()
            else:
                self.strength_raw = self.verify_DUP()
        else:
    ...
    @property
    def is_preserve_reading_frame(self):
        return sum(self.overlap_cds_len) % 3 == 0

    @property
    def new_stop_codon(self):
        if not self.is_preserve_reading_frame:
            # index = self.overlap_exon_index[-1]
            # return sum(self.transcript.cds_sizes[0:index])
            index = self.overlap_cds_index[-1]
            return sum(self.transcript.cds_sizes[0:index]) + self.overlap_exon_len[-1]
        else:
            return self.transcript.tx_length
    
    @property
    def is_undergo_NMD(self):
        if len(self.transcript.exon_sizes) == 1:
            return False
        
        elif len(self.transcript.exon_sizes) == 2:
            if self.transcript.exon_sizes[-2] <= 50:
                return False
            else:
                nmd_cutoff = sum(self.transcript.exon_sizes[:-1]) - 50

        else:
            if self.transcript.exon_sizes[-2] >= 50:
                nmd_cutoff = sum(self.transcript.exon_sizes[:-1]) - 50
            else:
                if self.transcript.exon_sizes[-3] >= 50:
                    nmd_cutoff = sum(self.transcript.exon_sizes[:-2]) - 50
                else:
                    nmd_cutoff = sum(self.transcript.exon_sizes[:-3])

        if self.new_stop_codon > nmd_cutoff:
            return False
        else:
            return True
    ...
    def verify_DEL(self):
        ...
        elif not self.is_preserve_reading_frame:
            if self.is_undergo_NMD:
    ...
    def verify_DUP(self):
        ...
        :return: Strength
        """
        if self.type in ['TDUP', 'DUP'] and not self.is_preserve_reading_frame and self.is_undergo_NMD:
            if self.type == 'TDUP':
                return Strength.VeryStrong
            else:
                return Strength.Strong
